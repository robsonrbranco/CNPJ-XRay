...
def test_manager_documenta_as_proprias_rotas(tmp_path):
    cfg = ConfigAPI(credenciais=tmp_path / "c.db", logs=tmp_path / "l",
                    estatisticas=tmp_path / "e.db", jwt_segredo="x")
    spec = manager.criar_app(cfg).openapi()

    assert "/manager/credenciais" in spec["paths"]
    assert "/manager/estatisticas" in spec["paths"]
    assert not any(c.startswith("/v2/") for c in spec["paths"])
